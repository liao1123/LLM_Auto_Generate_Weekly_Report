...
id = 你的学号,
advisor = 你的指导老师,
work = {1）明确毕设具体需求和功能已完成，形成需求分析文档v1初稿，内容涵盖主要功能模块和性能指标。\par 2）细化任务分解和时间安排正在进行中，已制定初步任务分解表，部分时间节点尚需调整。\par 3）关键技术和工具的确定已完成，选定了主要开发环境和辅助工具，为后续设计奠定基础。},
issue = {1）问题：需求部分细节不够明确，可能导致后续返工；思路：加强与导师沟通，定期确认需求细节，确保理解一致。\par 2）问题：任务分解时间安排部分节点不合理；思路：结合实际工作量调整时间计划，增加缓冲时间以应对突发情况。},
plan = {1）完善任务分解表，调整时间安排，确保每项任务合理分配。\par 2）根据需求分析文档，开始设计整体方案架构，准备设计方案文档初稿。\par 3）进一步调研关键技术细节，为技术选型报告提供支持和依据。},
