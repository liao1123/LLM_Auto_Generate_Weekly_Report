...
id = 你的学号,
advisor = 你的指导老师,
work = {1）完成详细设计文档v1的初稿，涵盖系统功能细节和接口定义，文档已提交评审；\par 2）完成模块划分图绘制，明确各功能模块职责及接口关系，便于后续开发；\par 3）制定模块开发计划，安排各模块开发优先级和时间节点，计划合理且可执行。},
issue = {1）问题：设计细节部分存在遗漏，导致部分接口定义不明确；思路：组织设计评审会议，邀请相关成员多轮审查完善设计文档；\par 2）问题：模块划分时部分功能边界模糊，可能影响开发效率；思路：通过讨论明确模块职责，调整划分方案确保模块独立性。},
plan = {1）根据评审反馈完善详细设计文档，补充遗漏细节，确保设计完整；\par 2）优化模块划分图，解决模块边界模糊问题，提升模块划分合理性；\par 3）开始环境搭建和工具配置，为后续模块开发做好准备，确保开发环境稳定。},
