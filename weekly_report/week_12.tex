...
id = 你的学号,
advisor = 你的指导老师,
work = {1）论文写作大纲已初步制定，涵盖绪论、相关工作、实验部分等主要章节，结构合理；\par 2）绪论和相关工作章节初稿完成，内容涵盖研究背景和文献综述，正在导师审阅中；\par 3）实验部分数据和图表整理中，已完成大部分数据汇总，图表制作正在进行。},
issue = {1）问题：论文结构安排不够合理导致章节衔接不顺畅；思路：多次与导师沟通，调整大纲结构，确保逻辑连贯；\par 2）问题：部分实验数据图表尚未完善；思路：加强数据整理，使用专业软件制作图表，提高图表质量。},
plan = {1）根据导师反馈，完善论文大纲，调整章节内容和顺序；\par 2）继续修改绪论和相关工作章节，提升文字表达和逻辑性；\par 3）开始撰写方法与实现章节，详细描述系统设计与实现过程，补充实验内容。},
