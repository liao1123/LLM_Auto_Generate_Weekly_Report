...
id = 你的学号,
advisor = 你的指导老师,
work = {1）答辩PPT终稿已完成，内容涵盖研究背景、方法、结果及总结，结构清晰完整；\par 2）完成了两次答辩模拟演练，重点练习了时间控制和关键问题回答；\par 3）根据模拟反馈调整了部分答辩内容，优化了表达逻辑和重点突出，提升了整体流畅度。},
issue = {1）问题：答辩表达部分环节不够流畅；思路：增加模拟次数，重点练习难点内容，提升表达自然度；\par 2）问题：部分答辩内容反馈不够突出重点；思路：针对反馈调整PPT和演讲稿，突出核心结论，简化冗余信息。},
plan = {1）继续进行至少两次答辩模拟，强化答辩表达能力和应对问题技巧；\par 2）完善答辩PPT和演讲稿，确保内容准确且重点突出；\par 3）准备答辩所需的相关材料，做好现场答辩的各项准备工作。},
